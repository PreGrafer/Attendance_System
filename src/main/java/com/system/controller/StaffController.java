...
public class StaffController {
    ...
    public void staffSignIn(HttpServletResponse response) throws Exception {
        ...
        if (isSignIn != 0) {
            ...
            try {
                timeProperties.load(this.getClass().getResourceAsStream("/time.properties"));
            } catch (IOException e) {
                e.printStackTrace();
            }
            //判断签到时间状态，9:00之前0-正常，之后1-迟到
            Date beginTime = dateFormat.parse(timeProperties.getProperty("signIn"));
    ...
    public void staffSignOut(HttpServletResponse response) throws Exception {
        ...
        if (isSignIn == 0) {
            ...
            //判断签退时间状态，18:00之后0-正常，之前1-早退
            Date endTime = dateFormat.parse(timeProperties.getProperty("signOut"));
            Calendar date = Calendar.getInstance();
            date.setTime(dateFormat.parse(time));
